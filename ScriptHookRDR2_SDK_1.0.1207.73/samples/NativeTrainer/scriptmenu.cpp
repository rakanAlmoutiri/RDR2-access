...
int MenuBase::OnInput()
{
	...
	// Edge Detection: تسجيل الحدث فقط عند انتقال الزر من unpressed إلى pressed
	bool shouldPlayBeep = false;
	if ((buttons.a && !m_previousButtonState.a) || 
	    (buttons.b && !m_previousButtonState.b) || 
	    (buttons.up && !m_previousButtonState.up) || 
	    (buttons.down && !m_previousButtonState.down))
	{
		shouldPlayBeep = true;
	}
	...
	if (shouldPlayBeep)
	{
		...
		waitTime = buttons.b ? 350 : 300;  // زيادة التأخير من 200/150 إلى 350/300
	}

	// معالجة الأزرار عند الضغطة الجديدة فقط
	if (buttons.a && !m_previousButtonState.a)
	{
		int activeItemIndex = GetActiveItemIndex();
		m_items[activeItemIndex]->OnSelect();
	} else
	if (buttons.b && !m_previousButtonState.b)
	{ 
		if (auto controller = GetController())
			controller->PopMenu();
	} else		
	if (buttons.up && !m_previousButtonState.up)
	{
	...
	if (buttons.down && !m_previousButtonState.down)
	{
		m_activeLineIndex++;
	...
	// حفظ الحالة الحالية للمقارنة في التكرار التالي
