...
public:
	...
	// Lightweight getters to allow external code to read labels without changing behavior
	string GetTitleCaption() const { return m_itemTitle ? m_itemTitle->GetCaption() : string(); }
	string GetActiveItemCaption() const {
		int idx = m_activeScreenIndex * MenuBase_linesPerScreen + m_activeLineIndex;
		if (idx >= 0 && idx < static_cast<int>(m_items.size()) && m_items[idx]) return m_items[idx]->GetCaption();
		return string();
	}
};
