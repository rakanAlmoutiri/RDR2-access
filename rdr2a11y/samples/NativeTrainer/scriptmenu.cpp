...
void MenuItemBase::SetStatusText(string text, int ms)
{
	...
	// Also speak status text for accessibility (non-intrusive)
	if (!text.empty()) {
...
int MenuBase::OnInput()
{
	...
	// Unified safe speaker for current focused item
	auto speakFocused = [this]() {
		int idx = GetActiveItemIndex();
		if (idx >= 0 && idx < static_cast<int>(m_items.size())) {
			MenuItemBase* it = m_items[idx];
			if (!it) return;
			std::string cap = it->GetCaption();
			if (cap.empty()) return;
			std::wstring wcap(cap.begin(), cap.end());
			A11y::speak(wcap, true);
		}
	};
	...
	if (buttons.a)
	{
		...
		MenuItemBase* item = m_items[activeItemIndex];
		item->OnSelect();
		// Speak state changes for switchable items (non-intrusive, on select only)
		if (item && item->GetClass() == eMenuItemClass::Switchable) {
			auto sw = static_cast<MenuItemSwitchable*>(item);
			std::string spoken = item->GetCaption();
			spoken += sw->GetState() ? " on" : " off";
			std::wstring wspoken(spoken.begin(), spoken.end());
			A11y::speak(wspoken, true);
		}
	} else
	if (buttons.b)
	{ 
	...
	// Speak current focused item AFTER beep and index update
	speakFocused();
	} else
	if (buttons.down)
	{
